...
It is supposed to show buffer "*Helm Help*", but it fails to switch to that buffer.
This problem is also in the old doom helm module.

This functionality works in both emacs prelude, and spacemacs, so it is not a helm bug.

To see helm documentation do:
